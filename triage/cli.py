...
from triage.jira_client import JiraAuthError, JiraClient, JiraConnectionError
...
@cli.command("generate-plan")
@click.option("--output", "-o", type=click.Path(), metavar="PATH", help="Save plan to file (default: stdout)")
@click.option(
    "--closure-rate", type=float, metavar="FLOAT", help="Previous day closure rate (0.0-1.0, e.g., 0.67 for 67%%)"
)
@click.option("--debug", is_flag=True, help="Enable detailed logging for debugging")
@click.pass_context
def generate_plan(ctx, output: Optional[str], closure_rate: Optional[float], debug: bool):
    ...
    try:
        ...
        if config.jira_project:
            click.echo(
                "📋 Fetching tasks from project " + click.style(config.jira_project, fg="green", bold=True) + "...",
                err=True,
            )
        ...
        click.echo(
            "   • Priorities: " + click.style(str(len(plan.priorities)), fg="green", bold=True) + " tasks", err=True
        )
        click.echo(
            "   • Admin: "
            + click.style(str(len(plan.admin_block.tasks)), fg="yellow")
            + f" tasks ({plan.admin_block.time_allocation_minutes} min)",
            err=True,
        )
        click.echo("   • Other: " + click.style(str(len(plan.other_tasks)), fg="white") + " tasks", err=True)
        ...
        if plan.previous_closure_rate is not None:
            ...
            click.echo("   • Previous closure: " + click.style(f"{rate_pct}%", fg=rate_color, bold=True), err=True)
...
@click.argument("task_key", metavar="TASK-KEY")
@click.option("--output", "-o", type=click.Path(), metavar="PATH", help="Save decomposition to file (default: stdout)")
@click.option("--auto-create", is_flag=True, help="Automatically create subtasks in JIRA (requires approval)")
@click.option("--debug", is_flag=True, help="Enable detailed logging for debugging")
@click.pass_context
def decompose(ctx, task_key: str, output: Optional[str], auto_create: bool, debug: bool):
    """Decompose a long-running task into daily-closable subtasks.

    This command analyzes a task and proposes a decomposition into smaller
    subtasks that can each be completed within one working day (≤8 hours).

    \b
    The decomposition includes:
      • Analysis of the parent task
      • Proposed subtasks with effort estimates
    ...
    \b
      # Analyze and propose decomposition
      $ triage decompose STRAT-11

    \b
      # Save decomposition to file
      $ triage decompose STRAT-11 -o decomposition.md

    \b
      # Auto-create subtasks in JIRA (with approval)
      $ triage decompose STRAT-11 --auto-create

    \b
      # Debug mode
      $ triage decompose STRAT-11 --debug
    """
    # Configure logging
    log_level = logging.DEBUG if debug else logging.INFO
    logging.basicConfig(
    ...
    logger.info(f"Starting task decomposition for {task_key}")
    ...
    if not is_valid:
        ...
        click.echo("❌ " + click.style("Configuration Error", fg="red", bold=True), err=True)
        click.echo(f"   {error_message}", err=True)
        click.echo("\n" + click.style("See:", fg="yellow") + " triage generate-plan --help", err=True)
        sys.exit(1)
    ...
    try:
        ...
        # Fetch the task
        click.echo("📋 Fetching task " + click.style(task_key, fg="green", bold=True) + "...", err=True)
        task = jira_client.get_task_by_key(task_key)

        if not task:
            click.echo("", err=True)
            click.echo("❌ " + click.style("Task Not Found", fg="red", bold=True), err=True)
            click.echo(f"   Task {task_key} does not exist or you don't have access to it", err=True)
            sys.exit(1)
        ...
        if classification.estimated_days <= 1.0:
            click.echo("", err=True)
            click.echo("ℹ️  " + click.style("Task Already Daily-Closable", fg="yellow", bold=True), err=True)
            click.echo(f"   {task_key} is estimated at {classification.estimated_days} days (≤1 day)", err=True)
            click.echo("   No decomposition needed", err=True)
            sys.exit(0)

        # Generate decomposition
        click.echo(
            "🔍 Analyzing task (estimated: "
            + click.style(f"{classification.estimated_days} days", fg="yellow")
            + ")...",
            err=True,
        )
        ...
        # Format output
        output_lines = [
            f"# Task Decomposition: {task_key}",
            "",
            f"**Parent Task:** {task.summary}",
            f"**Estimated Effort:** {classification.estimated_days} days",
            f"**Status:** {task.status}",
            f"**Priority:** {task.priority}",
            "",
            "---",
            "",
            f"## Proposed Subtasks ({len(subtasks)})",
            "",
        ]
        ...
        for i, subtask in enumerate(subtasks, 1):
            output_lines.extend(
                [
                    ...
                    subtask.description,
                    "",
                    f"**Estimated Effort:** {subtask.estimated_days} days",
                    "",
                ]
            )
        ...
        if output:
            ...
            click.echo("", err=True)
            click.echo(
                "✅ " + click.style("Decomposition saved to:", fg="green", bold=True) + f" {output_path}", err=True
            )
            logger.info(f"Decomposition written to file: {output_path}")
        else:
            click.echo()
            click.echo(markdown_output)
            logger.debug("Decomposition written to stdout")

        # Print summary
        click.echo("", err=True)
        click.echo("📊 " + click.style("Decomposition Summary", fg="blue", bold=True), err=True)
        click.echo(
            "   • Parent task: "
            + click.style(task_key, fg="green", bold=True)
            + f" ({classification.estimated_days} days)",
            err=True,
        )
        click.echo("   • Proposed subtasks: " + click.style(str(len(subtasks)), fg="green", bold=True), err=True)

        total_subtask_days = sum(st.estimated_days for st in subtasks)
        click.echo("   • Total subtask effort: " + click.style(f"{total_subtask_days} days", fg="yellow"), err=True)

        # Auto-create if requested
        if auto_create:
            click.echo("", err=True)
            click.echo("🤔 " + click.style("Create these subtasks in JIRA?", fg="yellow", bold=True), err=True)

            if click.confirm("   Proceed with creation?", err=True):
                click.echo("", err=True)
                click.echo("🔄 " + click.style("Creating subtasks...", fg="cyan"), err=True)

                created_keys = []
                for i, subtask in enumerate(subtasks, 1):
                    try:
                        subtask_key = jira_client.create_subtask(task_key, subtask)
                        created_keys.append(subtask_key)
                        click.echo(f"   ✓ Created {subtask_key}: {subtask.summary}", err=True)
                        logger.info(f"Created subtask {subtask_key}")
                    except Exception as e:
                        click.echo(f"   ✗ Failed to create subtask {i}: {e}", err=True)
                        logger.error(f"Failed to create subtask: {e}")

                click.echo("", err=True)
                click.echo(
                    "✅ " + click.style(f"Created {len(created_keys)} subtasks", fg="green", bold=True), err=True
                )
            else:
                click.echo("", err=True)
                click.echo("⏭️  Subtask creation cancelled", err=True)

        click.echo("", err=True)
        logger.info("Task decomposition completed successfully")
        ...
        click.echo(f"   {str(e)}", err=True)
        if not debug:
            click.echo("\n" + click.style("Tip:", fg="cyan") + " Run with --debug for more details", err=True)
        sys.exit(1)
