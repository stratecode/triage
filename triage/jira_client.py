...
from triage.models import JiraIssue, IssueLink, SubtaskSpec
...
class JiraClient:
    ...
    def create_subtask(self, parent_key: str, subtask: SubtaskSpec) -> str:
        ...
        parent_response = self._make_request_with_retry(
            'GET',
            f"{self.base_url}/rest/api/3/issue/{parent_key}",
            timeout=30
        )
        ...
        if parent_response.status_code == 410:
            ...
            parent_response = self._make_request_with_retry(
                'GET',
                f"{self.base_url}/rest/api/2/issue/{parent_key}",
                timeout=30
            )
    ...
    def detect_metadata_changes(self, previous_tasks: List[JiraIssue]) -> dict:
        ...
        for key, prev_task in previous_map.items():
            ...
            if prev_task.priority != curr_task.priority:
                task_changes['priority'] = {
                    'old': prev_task.priority,
                    'new': curr_task.priority
                }
            
            # Check story points changes
            if prev_task.story_points != curr_task.story_points:
                task_changes['story_points'] = {
                    'old': prev_task.story_points,
                    'new': curr_task.story_points
                }
            
            # Check time estimate changes
            if prev_task.time_estimate != curr_task.time_estimate:
                task_changes['time_estimate'] = {
                    'old': prev_task.time_estimate,
                    'new': curr_task.time_estimate
                }
            ...
            if prev_task.summary != curr_task.summary:
                task_changes['summary'] = {
                    'old': prev_task.summary,
                    'new': curr_task.summary
                }
    ...
    def detect_dependency_changes(self, previous_tasks: List[JiraIssue]) -> dict:
        ...
        for key, prev_task in previous_map.items():
            ...
            if added_link_ids or removed_link_ids:
                ...
                if added_link_ids:
                    added_links = [
                        link for link in curr_task.issue_links
                        if (link.link_type, link.target_key) in added_link_ids
                    ]
                ...
                if removed_link_ids:
                    removed_links = [
                        link for link in prev_task.issue_links
                        if (link.link_type, link.target_key) in removed_link_ids
                    ]
