...
from triage.models import DailyPlan, ApprovalResult, JiraIssue, SubtaskSpec
...
class ApprovalManager:
    ...
    def __init__(self, timeout_seconds: Optional[int] = None):
        ...
        logger.info(f"Approval manager initialized with timeout: {self.timeout_seconds}s ({self.timeout_seconds/3600:.1f} hours)")
        ...
        while True:
            ...
            if choice == '1':
                ...
                try:
                    ...
                    if (len(new_order) == len(plan.priorities) and 
                        set(new_order) == set(range(len(plan.priorities)))):
            ...
            if choice == '1':
                ...
                try:
                    ...
                    if 0 <= index < len(subtasks):
                        new_effort = input(f"Enter new effort in days (current: {subtasks[index].estimated_days:.2f}): ").strip()
                        ...
                        if new_effort_days > 1.0:
                            print("⚠️  Warning: Subtask effort exceeds 1 day. This violates the daily-closable constraint.")
