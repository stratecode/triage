...
def jira_issue_with_dependencies_strategy(draw):
    ...
    if dependency_type in ["issue_links", "both"]:
        ...
        for _ in range(num_links):
            ...
            number = draw(st.integers(min_value=1, max_value=9999))
            target_key = f"{project}-{number}"
            blocking_links.append(IssueLink(
...
def test_property_4_classification_idempotence(issue: JiraIssue):
    ...
    assert classification1.category == classification2.category == classification3.category, \
        "Category must be the same across multiple classifications"
    
    assert classification1.is_priority_eligible == classification2.is_priority_eligible == classification3.is_priority_eligible, \
        "is_priority_eligible must be the same across multiple classifications"
    
    assert classification1.has_dependencies == classification2.has_dependencies == classification3.has_dependencies, \
        "has_dependencies must be the same across multiple classifications"
    
    assert classification1.estimated_days == classification2.estimated_days == classification3.estimated_days, \
        "estimated_days must be the same across multiple classifications"
    
    assert classification1.blocking_reason == classification2.blocking_reason == classification3.blocking_reason, \
...
def test_property_27_dependency_detection_completeness(issue: JiraIssue):
    ...
    if expected_has_dependencies:
        assert classification.has_dependencies, \
    ...
    if classification.has_dependencies:
        assert not classification.is_priority_eligible or classification.category == TaskCategory.DEPENDENT, \
...
def test_property_5_administrative_task_marking(issue: JiraIssue):
    ...
    has_admin_issue_type = issue.issue_type in admin_issue_types or \
                           any(keyword in issue.issue_type.lower() for keyword in ['admin', 'approval', 'review'])
    ...
    if expected_is_admin:
        ...
        assert actual_is_admin, \
            f"Task with admin markers was not detected as administrative. Labels: {issue.labels}, Type: {issue.issue_type}"
        ...
        if classification.category == TaskCategory.ADMINISTRATIVE:
            assert not classification.is_priority_eligible, \
                "Administrative tasks should not be priority eligible"
