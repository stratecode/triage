...
def test_property_11_long_running_task_identification(
    estimated_days, key_prefix, key_number, summary, description,
...
def test_property_12_decomposition_subtask_constraints(
    ...
    for i, subtask in enumerate(subtasks):
        ...
        assert subtask.estimated_days > 0, \
            f"Subtask {i+1} has non-positive effort: {subtask.estimated_days}"
...
def test_property_14_decomposition_approval_requirement(
    ...
    assert not mock_jira_client.create_subtask.called, \
    ...
    assert all(isinstance(s, SubtaskSpec) for s in subtasks), \
