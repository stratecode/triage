...
def test_property_30_closure_rate_calculation(data):
    ...
    assert 0.0 <= closure_rate <= 1.0, f"Closure rate {closure_rate} is outside valid range [0.0, 1.0]"
...
def test_property_31_closure_rate_display(previous_closure_rate):
    ...
    assert (
        plan.previous_closure_rate == previous_closure_rate
    ...
    assert "## Previous Day" in markdown, "Markdown output does not contain 'Previous Day' section"

    # Verify markdown contains "Closure Rate"
    assert "Closure Rate:" in markdown, "Markdown output does not contain 'Closure Rate:' text"

    # Verify markdown contains percentage
    percentage = int(previous_closure_rate * 100)
    assert f"({percentage}%)" in markdown, f"Markdown output does not contain expected percentage ({percentage}%)"

    # Verify markdown contains task completion information (X/Y tasks completed)
    assert "tasks completed" in markdown, "Markdown output does not contain 'tasks completed' text"
