...
class TestBlockingTaskDetection:
    ...
    def test_blocking_task_detection(self, blocking_tasks):
        ...
        assert len(queued_operations) == len(blocking_tasks), \
        ...
        for (args, kwargs) in queued_operations:
            assert kwargs.get('operation_type') == 'handle_blocking_task', \
                "Operation type should be 'handle_blocking_task'"
            assert kwargs.get('priority') == OperationPriority.BLOCKING, \
                "Priority should be BLOCKING"
        ...
        assert queued_task_keys == expected_task_keys, \
    ...
    def test_blocking_task_detection_with_polling(self, blocking_tasks):
        ...
        assert len(check_called) > 0, \
            "Blocking task check should be called during polling"
        
        # Verify: fetch_blocking_tasks should have been called
        assert mock_jira_client.fetch_blocking_tasks.called, \
            "fetch_blocking_tasks should be called during polling"
...
class TestOperationPriorityOrdering:
    ...
    @given(
        num_blocking=st.integers(min_value=1, max_value=5),
        num_normal=st.integers(min_value=1, max_value=5)
    )
    @settings(max_examples=20, deadline=None)
    def test_operation_priority_ordering(self, num_blocking, num_normal):
        ...
        assert len(execution_order) == total_operations, \
    ...
    @given(operations=st.lists(
        st.tuples(
            st.sampled_from(['blocking', 'normal']),
    ...
    def test_priority_ordering_with_mixed_queue(self, operations):
        ...
        for op_type, op_id in operations:
            ...
            scheduler.queue_operation(
                operation_type=f"{op_type}_{op_id}",
                callback=callback,
                priority=priority,
                op_type=op_type,
                op_id=op_id
        ...
        assert len(execution_order) == len(operations), \
        ...
        if blocking_count > 0 and normal_count > 0:
            ...
            assert all(op_type == 'blocking' for op_type, _ in first_n), \
            ...
            assert all(op_type == 'normal' for op_type, _ in remaining), \
