...
from triage.models import IssueLink, JiraIssue, TaskCategory
    ...
    def test_status_changes_reflected_in_plan(self, initial_tasks, status_change_indices):
        ...
        with patch.object(JiraClient, "fetch_active_tasks") as mock_fetch:
            ...
            for idx in status_change_indices:
                ...
                assert (
                    completed_key not in plan2_keys
    ...
    def test_priority_changes_reflected_in_plan(self, initial_tasks, priority_change_index):
        ...
        with patch.object(JiraClient, "fetch_active_tasks") as mock_fetch:
            ...
            if task1_classification and task2_classification:
                assert (
                    task1_classification.task.priority != task2_classification.task.priority
                ), "Priority change should be reflected in subsequent plan"
                assert task2_classification.task.priority == new_priority, f"New priority should be {new_priority}"
...
class TestDependencyReEvaluation:
    ...
    @given(task_with_dependency=jira_issue_strategy(), dependency_resolved=st.booleans())
    @settings(max_examples=100, deadline=None)
    def test_resolved_dependencies_make_task_eligible(self, task_with_dependency, dependency_resolved):
        ...
        blocking_link = IssueLink(link_type="is blocked by", target_key="PROJ-999", target_summary="Blocking task")
        ...
        assert classification1.has_dependencies, "Task should have dependencies initially"
        assert not classification1.is_priority_eligible, "Task with dependencies should not be priority eligible"
        ...
        if dependency_resolved:
            ...
            assert not classification2.has_dependencies, "Task should not have dependencies after resolution"
    ...
    @given(initial_tasks=task_list_strategy(min_size=2, max_size=5))
    @settings(max_examples=100, deadline=None)
    def test_dependency_resolution_reflected_in_plan(self, initial_tasks):
        ...
        task_with_dep = JiraIssue(
            ...
            labels=[],
            issue_links=[
                IssueLink(link_type="is blocked by", target_key=blocking_task.key, target_summary=blocking_task.summary)
            ],
            custom_fields={},
        )
        ...
        with patch.object(JiraClient, "fetch_active_tasks") as mock_fetch:
            ...
            assert task_with_dep.key not in plan1_priority_keys, "Task with dependency should not be in priorities"
            ...
            if task_classification:
                # Verify task no longer has dependencies
                assert not task_classification.has_dependencies, "Task should not have dependencies in second plan"

                # Verify task is now priority eligible
                assert (
                    task_classification.is_priority_eligible
