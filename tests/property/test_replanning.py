...
def test_property_9_replanning_trigger(plan_and_tasks):
    ...
    assert hasattr(plan_generator, 'generate_replan'), \
    ...
    assert blocking_task.key in new_priority_keys or new_priority_keys != current_priority_keys, \
...
def test_property_10_blocking_task_inclusion(plan_and_tasks):
    ...
    assert len(new_plan.priorities) <= 3, \
        f"Re-plan has {len(new_plan.priorities)} priorities, but maximum is 3"
